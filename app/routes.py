...
def token_required(f):
    ...
    def decorator(*args, **kwargs):
        ...
        try:
            ...
            current_user = session.query(Accounts).filter_by(
...
def register():
    """The user registration API creates a user account in  application.
    ---
    post:
        ...
        200:
          ...
          content:
            application/json:
              message: String
        400:
          description: Return a error message
          content:
            application/json:
              message: String
    """
    ...
    user = session.query(Accounts).filter(or_(Accounts.username == data.get(
        "username"), Accounts.email == data.get("email"))).first()
    if user:
...
def login_user():
    """The user login API allow to login to application.
     ---
     post:
       description: Login
       responses:
         ...
         400:
           ...
           content:
             application/json:
               message: String
        401:
           description: Return invalid credentials
           content:
             application/json:
               message: String
    """
    ...
    if user and check_password_hash(user.password, auth.password):
        ...
        ) + datetime.timedelta(minutes=45)}, Config.SECRET_KEY, "HS256")
...
def add_basic_info(current_user):
    """API allow user to add basic info to resume.
     ---
     post:
       description: Basic information for user
       responses:
         200:
    ...
    if not resume:
        engine.execute(resume_table.insert(), public_id=current_user.public_id, data={
                       "basic_info": data})
    else:
        updated_resume = resume.data
        updated_resume.update({"basic_info": data})
        engine.execute(resume_table.update(),
                       public_id=current_user.public_id, data=updated_resume)
...
def add_work_experience(current_user):
    """API allow user to add work experience list to resume.
     ---
     post:
       description: Work experience list for user
       responses:
         200:
           description: Return success message
           content:
...
def add_education(current_user):
    """API allow user to add education list to resume.
     ---
     post:
       description: Education list for user
       responses:
         200:
           description: Return success message
           content:
...
def add_certificates(current_user):
    """API allow user to add certificates list to resume.
     ---
     post:
       description: Certificates list for user
       responses:
         200:
           description: Return success message
           content:
...
def add_others(current_user):
    """API allow user to add others tab to resume.
     ---
     post:
       description: Others tab for user
       responses:
         200:
           description: Return success message
           content:
    ...
    if not resume:
        engine.execute(resume_table.insert(),
                       public_id=current_user.public_id, data={"others": data})
    else:
        updated_resume = resume.data
...
def get_resume(current_user):
    """API allow user to get resumes information.
     ---
     get:
       description: Resume information
       responses:
         200:
           description: Return resume information object
           content:
             application/json:
               result: Object
        401:
